...
#!/usr/bin/env python3
"""
Super Memory — client connectivity test.
Server: {server_url}
...
#!/usr/bin/env python3
"""
Super Memory — client auto-setup script.
Generated by: {server_url}/client-setup

What this script does:
  1. Creates ~/supermemory-client/ directory
...
HOME       = Path.home()
CLIENT_DIR = HOME / "supermemory-client"
...
if claude_bin is None:
    ...
    print(f"    claude mcp add -s user -e MEMORY_SERVER_URL={{SERVER_URL}} super-memory -- {{python}} {{server_py}}")
else:
    # Remove existing entry if present (ignore errors)
    subprocess.run([claude_bin, "mcp", "remove", "super-memory", "-s", "user"],
                   capture_output=True)
    ...
    if result.returncode == 0:
        log("Registered as 'super-memory' (user scope)")
    else:
        # Old syntax fallback
