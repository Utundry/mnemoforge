...
_DEFAULT_INSTINCTS: dict[str, dict[str, Any]] = {
    "trust_first": {
        ...
        "why_it_matters": "SuperMemory can remain useful while parts of the substrate are degraded or contaminated.",
    ...
    "ask_memory_before_code": {
        ...
        "why_it_matters": "SuperMemory should narrow the search space before the agent spends effort reading code.",
    ...
    "real_usage_beats_lab_confidence": {
        ...
        "why_it_matters": "For SuperMemory, real operation reveals product gaps that synthetic confidence hides.",
    ...
    "system_must_explain_itself": {
        ...
        "action": "Expose onboarding, playbooks, readiness, workflow summaries, and self-describing operational guidance from within SuperMemory.",
        "why_it_matters": "Agents should learn how to use SuperMemory from SuperMemory itself.",
    ...
    "unified_mcp_surface_first": {
        ...
        "action": "Prefer unified MCP surfaces such as normalize_mcp_intent, list_open_tasks, reopen_task, report_task_checkpoint, list_artifacts, enrich-task, and review_improvement. For open work items, use list_open_tasks first; use reopen_task when you need to resume an existing task; use list_artifacts when you need broader artifact filtering. If you are unsure which MCP family to use, call normalize_mcp_intent first, then list_tool_families or tool_recommend before loading the full catalog. When any MCP tool is in testing, finish that tool's use-case with tool_feedback and include a compact evaluation envelope with scope, what_was_tested, expected_behavior, observed_behavior, friction, suggestion, and next_action when available. For task work, record a checkpoint at planning and every meaningful stage transition with report_task_checkpoint so interruptions do not lose progress. After completing a task or implementation, report the outcome back to SuperMemory with the linked improvement/task id, stage, and verdict when relevant; do not leave completion only in chat. Testing tools are auto-seeded for review and may be promoted or deprecated by background lifecycle review after enough time or feedback. Use specialized endpoints only when the unified surface cannot express the need; do not read project tables directly in agent workflows.",
    ...
    "progressive_disclosure_for_mcp_depth": {
        ...
        "why_it_matters": "SuperMemory stays useful when it narrows the path first and expands only on demand.",
    ...
    "every_task_must_exist_in_memory": {
        ...
        "action": "Register the task in SuperMemory as a first-class work item before or at the start of execution.",
    ...
    "capture_task_checkpoints_across_lifecycle": {
        ...
        "action": "Record a compact task checkpoint at planning and every meaningful transition. Include stage, current status, summary, blockers, and next step. If the agent stops unexpectedly, the last checkpoint must still be recoverable in SuperMemory.",
    ...
    "close_with_outcome_and_followups": {
        ...
        "action": "Record outcome, final decision, what worked, what failed, and follow-up tasks in SuperMemory.",
