...
UNSAFE_OVERRIDE_ENV = "MNEMOFORGE_ALLOW_UNSAFE_LIVE_TESTS"
TEST_TARGETS_ENV = "MNEMOFORGE_DB_TEST_TARGETS"
LIVE_TARGETS_ENV = "MNEMOFORGE_LIVE_TARGETS"
LEGACY_UNSAFE_OVERRIDE_ENV = "SUPERMEMORY_ALLOW_UNSAFE_LIVE_TESTS"
LEGACY_TEST_TARGETS_ENV = "SUPERMEMORY_DB_TEST_TARGETS"
LEGACY_LIVE_TARGETS_ENV = "SUPERMEMORY_LIVE_TARGETS"
...
def _env_value(env_name: str, legacy_env_name: str = "") -> str:
    return os.getenv(env_name, "") or (os.getenv(legacy_env_name, "") if legacy_env_name else "")


def _configured_targets(env_name: str, legacy_env_name: str = "") -> set[tuple[str, str]]:
    raw = _env_value(env_name, legacy_env_name)
...
def is_allowed_db_test_target(server_url: str) -> bool:
    allowed_targets = _configured_targets(TEST_TARGETS_ENV, LEGACY_TEST_TARGETS_ENV)
    return bool(allowed_targets) and _target_tuple(server_url) in allowed_targets


def is_live_like_target(server_url: str) -> bool:
    live_targets = _configured_targets(LIVE_TARGETS_ENV, LEGACY_LIVE_TARGETS_ENV)
...
def assert_db_backed_test_target(server_url: str, *, context: str = "DB-backed test") -> None:
    ...
    if _env_value(UNSAFE_OVERRIDE_ENV, LEGACY_UNSAFE_OVERRIDE_ENV).strip().lower() in {"1", "true", "yes"}:
        return

    host, port = _target_tuple(server_url)
    if is_live_like_target(server_url):
        reason = "target looks like the live MnemoForge server"
    elif not _configured_targets(TEST_TARGETS_ENV, LEGACY_TEST_TARGETS_ENV):
