...
async def test_handoff_can_use_human_readable_label_and_pickup_by_it(client, monkeypatch):
    ...
    assert create_body["project_id"] == "mnemoforge"
    ...
    assert pickup_body["handoffs"][0]["project_id"] == "mnemoforge"
    ...
    assert "project_id: mnemoforge" in pickup_body["handoffs"][0]["content"]
...
async def test_handoff_payload_keeps_full_packet_in_sqlite_and_link_in_qdrant(client, monkeypatch):
    ...
    assert (metadata.get("meta") or {}).get("project_id") == "mnemoforge"
...
async def test_background_handoffs_fall_back_to_sqlite_when_qdrant_scroll_fails(client, monkeypatch):
    ...
    create_resp = await client.post(
        ...
        json={
            ...
            "background_payload": {"project": "mnemoforge", "force": False},
...
async def test_expand_handoff_refs_returns_compact_resolution(client, monkeypatch):
    ...
    assert data["project_id"] == "mnemoforge"
...
async def test_refresh_handoff_context_rebuilds_summary_and_refs(client, monkeypatch):
    ...
    async def fake_assemble_project_context(**kwargs):
        assert kwargs["project_id"] == "mnemoforge"
...
async def test_handoff_views_surface_project_task_recommendations(client, monkeypatch):
    ...
    assert pickup_data["project_task_recommendations"]["mnemoforge"]["recommended_task_id"] == "task-next"
    ...
    assert list_data["project_task_recommendations"]["mnemoforge"]["recommended_task_id"] == "task-next"
    ...
    assert summary_resp.status_code == 200, summary_resp.text
    summary = summary_resp.json()
    assert summary["project_task_recommendations"]["mnemoforge"]["recommended_task_id"] == "task-next"
...
async def test_dispatch_background_task_packet_submits_supported_job(client, monkeypatch):
    ...
    assert fake_queue.calls == [("docs_rebuild", {"project": "mnemoforge", "force": False})]
...
async def test_reconcile_background_task_packet_closes_done_job(client, monkeypatch):
    ...
    class _FakeQueue:
        ...
        async def submit(self, job_type: str, payload: dict) -> str:
            ...
            self.jobs[job_id] = {
                ...
                "job_type": job_type,
                "status": "done",
                "result": {"project": "mnemoforge", "sections": ["overview", "api"]},
            }
            return job_id
    ...
    assert "project=mnemoforge" in data["verification_summary"]
