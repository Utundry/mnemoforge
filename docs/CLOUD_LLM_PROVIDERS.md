...
SloplessCode supports configurable external LLM profiles with explicit API style selection.
...
- If `CLOUD_LLM_API_KEY` is set, SloplessCode uses `CLOUD_LLM_PROVIDER`, `CLOUD_LLM_MODEL`, and `CLOUD_LLM_BASE_URL`.
- If `CLOUD_LLM_API_KEY` is empty, SloplessCode falls back to legacy `GLM_*`.
